...
class Connection extends EventEmitter {
    ...
    _resUntagged(info) {
        ...
        let attrs;
        let key;
        //this.emit("type", this._box, info, this._curReq);
        if (type === "bye") this._sock.end();
            ...
            if (this._box) {
                ...
                if (now > prev && this.state === "authenticated") {
                    if (this._box.uidnext != null) ++this._box.uidnext;
                    this._box.messages.new = now - prev;
                    this.emit("mail", this._box.messages.new, this._box, info);
                }
            }
    ...
    _authCRAMMD5(secret) {
        ...
        const hmac = crypto.createHmac("md5", this._config.password).update(decodedSecret).digest("hex");
        const response = new Buffer(this._config.user + " " + hmac).toString("base64");
