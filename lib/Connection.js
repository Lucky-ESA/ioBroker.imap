    ...
    connect() {
        ...
        function onconnect() {
            ...
            self._tmrAuth = setTimeout(function () {
                ...
                self.emit("error", err, self.client);
                socket.destroy();
            }, config.authTimeout);
        }
        ...
        if (type === "bye") this._sock.end();
            ...
            if (this._box) {
                ...
                if (now > prev && this.state === "authenticated") {
                    ...
                    if (this._box.uidnext != null) this._box.uidnext + this._box.messages.new;
                    if (this._box.reason === "recent" && this._box.seq === this._box.messages.new) {
            ...
            if (/^(?:UID )?FETCH/.test(this._curReq.fullcmd)) {
                ...
                this.emit("update", info.num, info.text, this.client);
            }
        } else if (type !== "") {
