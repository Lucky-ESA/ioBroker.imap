...
module.exports = {
    ...
    async onReadyImap(box, client) {
        ...
        this.clients[client].openBox(box, false, async (error, mailbox) => {
            if (error) {
                ...
                ++this.countOnline;
                this.log.info("COUNTERPLUS: " + this.countOnline);
    ...
    onCloseImap(error, clientID) {
        ...
        --this.countOnline;
        this.log.info("COUNTERMINUS: " + this.countOnline);
     ...
     * @param {string} clientID client id
     */
    onErrorImap(err, clientID) {
        this.log_translator("warn", "Error", clientID, err);
    },
    /**
    ...
    async onMail(count, box, info, client) {
        ...
        let nextuid = this.diffArray(this.clientsRaw[client][inbox], new_mail, 2);
        if (nextuid.length === 0) {
            nextuid = [box.uidnext];
        }
        if (this.clients[client].serverSupports("SORT")) {
    ...
    change_events(client, event, uid, box) {
        this.imap[client][event](uid, box, (err) => {
            if (!err) {
