...
module.exports = {
    ...
    changesearch(client, search) {
        try {
            const state = search != null && search.val != null ? search.val : JSON.stringify(["ALL"]);
            this.flag = typeof state == "string" ? JSON.parse(state) : state;
        } catch (e) {
            this.flag = ["ALL"];
        }
        if (this.clients[client].serverSupports("SORT")) {
            ...
            if (attrs && attrs.struct) {
                for (const att of attrs.struct) {
                    if (Array.isArray(att)) {
                        this.log.info("ANHANG: " + JSON.stringify(att));
