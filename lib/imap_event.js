...
module.exports = {
    ...
    async onReadyImap(box, client) {
        ...
        this.clients[client].openBox(box, false, async (error, mailbox) => {
            if (error) {
                ...
                await this.loadAllSeqno(client, "INBOX", false);
        ...
        this.clients[client].sort(["-ARRIVAL"], this.clientsRaw[client].flag, async (error, results) => {
            if (error) {
                ...
                for (const result of results) {
                    if (count > this.clientsRaw[client].max) {
                        break;
                    }
    ...
    onExpunge(seqno, client) {
        ...
        this.updateTotal(client, this.clientsRaw[client].inbox_activ);
    },
    /**
        ...
        return new Promise(resolve => {
            this.clients[client].search(["ALL"], async (error, results) => {
                if (error) {
                    this.log_translator("error", "Error", client, error);
                    if (!second) {
                        this.clientsRaw[client][inbox] = [];
                    }
                    resolve([]);
                }
                if (!second) {
                    this.clientsRaw[client][inbox] = results;
                }
                resolve(results);
            });
        });
    ...
    async setTotal(client, inbox) {
        ...
        await this.setState(`${client}.total`, {
            val: count_all,
            ack: true,
        });
        await this.setState(`${client}.total_unread`, {
            val: count_unseen,
            ack: true,
        });
    },
    ...
    async onMail(count, box, info, client) {
        ...
        this.log_translator("debug", "logs", `inbox: ${inbox}`);
        this.log_translator("debug", "logs", `clientsRaw: ${this.clientsRaw[client][inbox]}`);
        this.log_translator("debug", "logs", `uidnext: ${box.uidnext}`);
        this.log_translator("debug", "logs", `new_mail: ${new_mail}`);
        const uidNext = this.clientsRaw[client][inbox] != null ? this.clientsRaw[client][inbox] : [box.uidnext - 1];
        let nextuid = this.diffArray(uidNext, new_mail, 2, client);
        this.clientsRaw[client][inbox] = new_mail;
        if (nextuid === 0 || nextuid.length === 0) {
            nextuid = [box.uidnext];
        }
        this.log_translator("debug", "logs", `nextuid: ${nextuid}`);
        if (this.clients[client].serverSupports("SORT")) {
    ...
    newMails(client, uid, what, sort) {
        this.log_translator("debug", "logs", `sort: ${sort}`);
        this.log_translator("debug", "logs", `newMail: ${JSON.stringify(uid)}`);
     ...
     *
     * @param {string} client device id
     * @param {string} folder actual inbox
     */
    async updateTotal(client, folder) {
        this.log.error(folder);
        this.totalTimeout[client] && this.clearTimeout(this.totalTimeout[client]);
        this.totalTimeout[client] = null;
        this.totalTimeout[client] = this.setTimeout(async () => {
            const all_mail = await this.loadAllSeqno(client, folder, true);
            await this.setState(`${client}.total`, {
                val: typeof all_mail === "object" ? all_mail.length : 0,
                ack: true,
            });
        }, 5 * 1000);
    },
     ...
     * @param {string} folder
     */
    addBox(client, folder) {
        if (!folder || folder == "") {
            return;
        }
        this.clients[client].addBox(folder, err => {
            if (!err) {
                this.log_translator("info", "addBox", `addBox ${folder} success`, client);
                this.setFolder(client);
            } else {
                this.log_translator("error", "Error", client, err);
            }
     ...
     * @param {string} folder
     */
    deleteBox(client, folder) {
        if (!folder || folder == "") {
            return;
        }
        this.clients[client].delBox(folder, err => {
            if (!err) {
                this.log_translator("info", "delBox", `deleteBox ${folder} success`, client);
                this.setFolder(client);
            } else {
                this.log_translator("error", "Error", client, err);
            }
    ...
    rename(client, oldname, newname) {
        ...
        this.clients[client].renameBox(oldname, newname, err => {
            if (!err) {
                this.log_translator("info", "renameBox", `renameBox ${oldname} with ${newname} success`, client);
                this.setFolder(client);
            } else {
                this.log_translator("error", "Error", client, err);
            }
    ...
    changeFolder(client, folder) {
        if (folder == this.clientsRaw[client].inbox_activ) {
            return;
        }
        if (folder && folder != "") {
            ...
            this.clients[client].openBox(folder, false, async (error, mailbox) => {
                if (error) {
                    ...
                    await this.loadAllSeqno(client, this.clientsRaw[client].inbox_activ, false);
                    if (this.clients[client].serverSupports("SORT")) {
                        this.readMailsSort(client);
                    } else {
                        this.readMails(client);
