...
module.exports = {
            ...
            try {
                ...
                f.on("message", (msg, seqno) => {
                    ...
                    msg.once("attributes", (att) => {
                        this.log.info("ATTRSS: " + JSON.stringify(att));
                        attrs = att;
                    });
                    this.log.info("ATTRS: " + JSON.stringify(attrs));
                    if (attrs && Object.keys(attrs.struct).length > 1) {
                        msg.on("body", async (stream) => {
                            const mail = await simpleParser(stream, this.clientsRaw[client].node_option);
                            stream.resume();
                            const d = this.clients[client].fetch(result, {
                                bodies: ["TEXT"],
                            });
                            d.on("message", (msg_text) => {
                                msg_text.on("body", async (stream_text, info) => {
                                    const mail_text = await simpleParser(
                                        stream_text,
                                        this.clientsRaw[client].node_option,
                                    );
                                    mail.text = mail_text.text;
                                    mail.html = mail_text.html;
                                    mail.textAsHtml = mail_text.textAsHtml;
                                    await this.updateattachments(mail, attrs);
                        ...
                        e.on("message", (msg, seqno) => {
                            msg.on("body", async (stream, info) => {
                                const mail_all = await simpleParser(stream, this.clientsRaw[client].node_option);
                                await this.updateattachments(mail_all, attrs);
                                this.setMail(mail_all, seqno, attrs, info, client, counter, all, sort);
                                stream.resume();
                                resolve(true);
                            });
                        });
        ...
        return new Promise((resolve) => {
            try {
                ...
                f.on("message", (msg, seqno) => {
                    ...
                    this.log.info("ATT: " + JSON.stringify(attrs));
                    msg.on("body", async (stream, info) => {
                        this.log.info("STREAM: " + JSON.stringify(stream));
                        const mail = await simpleParser(stream, this.clientsRaw[client].node_option);
                        await this.updateattachments(mail, attrs);
                        this.setMail(mail, seqno, attrs, info, client, counter, all, sort);
                        stream.resume();
    ...
    newMails(client, uid, what, sort) {
        ...
        for (const uids of uid) {
            ...
            let attrs;
            if (this.clientsRaw[client].att) {
                const a = this.clients[client].fetch(uids, {
                    bodies: ["HEADER.FIELDS (FROM TO SUBJECT DATE)"],
                    struct: true,
                });
                a.on("message", (msg) => {
                    ...
                    if (attrs && Object.keys(attrs.struct).length > 1) {
                        msg.on("body", async (stream) => {
                            const mail_header = await simpleParser(stream, this.clientsRaw[client].node_option);
                            stream.resume();
                            const d = this.clients[client].fetch(uids, {
                                bodies: ["TEXT"],
                            });
                            d.on("message", (msg_text, seqno) => {
                                ...
                                msg_text.on("body", async (stream_text, info) => {
                                    ...
                                    mail_header.text = mail_text.text;
                                    mail_header.html = mail_text.html;
                                    mail_header.textAsHtml = mail_text.textAsHtml;
                                    await this.updateattachments(mail_header, attrs);
                                    this.updatejson(mail_header, seqno, attrs, info, client, what);
                                    stream.resume();
                                });
                            });
                            d.once("error", (error) => {
     ...
     * @param {object} attrs
     */
    async updateattachments(mail, attrs) {
        const att_data = {
            partID: 0,
            id: null,
            uid: 0,
        ...
        if (attrs && attrs.struct) {
            ...
            mail.attachments = counter_att;
            mail.attachments_info = all_att;
    ...
    updatemail(client, seq, what) {
        try {
            ...
            let attrs;
            if (this.clientsRaw[client].att) {
                const a = this.clients[client].seq.fetch(seq, {
                    bodies: ["HEADER.FIELDS (FROM TO SUBJECT DATE)"],
                    struct: true,
                });
                a.on("message", (msg) => {
                    ...
                    if (attrs && Object.keys(attrs.struct).length > 1) {
                        msg.on("body", async (stream) => {
                            const mail_header = await simpleParser(stream, this.clientsRaw[client].node_option);
                            stream.resume();
                            const d = this.clients[client].seq.fetch(seq, {
                                bodies: ["TEXT"],
                            });
                            d.on("message", (msg_text, seqno) => {
                                msg_text.on("body", async (stream_text, info) => {
