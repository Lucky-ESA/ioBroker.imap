module.exports = {
    ...
    async createinbox(id, folder) {
        ...
        common = {
            type: "string",
            role: "state",
            name: this.helper_translator("create mailbox folder"),
            desc: "Create by Adapter",
            read: true,
            write: true,
            def: "",
        };
        await this.createDataPoint(`${id}.remote.mailbox_folder_create`, common, "state");
        common = {
            type: "string",
            role: "state",
            name: this.helper_translator("delete mailbox folder"),
            desc: "Create by Adapter",
            read: true,
            write: true,
            def: "",
        };
        await this.createDataPoint(`${id}.remote.mailbox_folder_delete`, common, "state");
        common = {
            type: "string",
            role: "state",
            name: this.helper_translator("change name mailbox folder"),
            desc: "Create by Adapter",
            read: true,
            write: true,
            def: '["oldname", "newname"]',
        };
        await this.createDataPoint(`${id}.remote.mailbox_folder_change_name`, common, "state");
    },
    async createCounter() {
        let common = {};
        common = {
    ...
    async createQuota(id, info) {
        ...
        if (info.data.storage.usage != null) {
            ...
            await this.setState(`${id}.infos.storage.storage_usage`, {
        ...
        if (info.data.storage.limit != null) {
            ...
            await this.setState(`${id}.infos.storage.storage_limit`, {
