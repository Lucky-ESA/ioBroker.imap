...
class MailListener extends EventEmitter {
    ...
    //Not implemented yet
    set_addLabels(uid, flag) {
        this.imap.addLabels(uid, flag, (err) => {
            if (!err) {
                this.emit("log", "info", "SetFlag", `${uid}: addLabels success`, this.client);
            } else {
                this.emit("error", err, this.client);
            }
        });
    ...
    //Not implemented yet
    set_delLabels(uid, flag) {
        this.imap.delLabels(uid, flag, (err) => {
            if (!err) {
                this.emit("log", "info", "SetFlag", `${uid}: delLabels success`, this.client);
            } else {
                this.emit("error", err, this.client);
            }
        });
    ...
    //Not implemented yet
    set_setLabels(uid, flag) {
        this.imap.setLabels(uid, flag, (err) => {
            if (!err) {
                this.emit("log", "info", "SetFlag", `${uid}: setLabels success`, this.client);
            } else {
                this.emit("error", err, this.client);
            }
        });
