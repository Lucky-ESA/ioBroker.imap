...
class MailListener extends EventEmitter {
    constructor(options, adapter) {
        ...
        try {
            ...
            this.adapter.log.debug("cannot tlsoption parse: " + e);
        }
        this.all_seqno = [];
        this.imap = new Imap({
    ...
    async subscribeBox(mailboxname) {
        this.adapter.log.info("SUB: " + mailboxname);
    ...
    set_move(seqno, box) {
        this.imap.move(seqno, box, (err) => {
            if (!err) {
                this.adapter.log.info(seqno + ": move success");
            } else {
                this.emit("error", err, this.client);
            }
        });
    ...
    set_copy(seqno, box) {
        this.imap.copy(seqno, box, (err) => {
            if (!err) {
                this.adapter.log.info(seqno + ": copy success");
            } else {
                this.emit("error", err, this.client);
            }
        });
    ...
    set_addFlags(seqno, flag) {
        this.imap.addFlags(seqno, flag, (err) => {
            if (!err) {
                this.adapter.log.info(seqno + ": addFlags success");
            } else {
                this.emit("error", err, this.client);
            }
        });
    ...
    set_delFlags(seqno, flag) {
        this.imap.delFlags(seqno, flag, (err) => {
            if (!err) {
                this.adapter.log.info(seqno + ": delFlags success");
            } else {
                this.emit("error", err, this.client);
            }
        });
    ...
    set_setFlags(seqno, flag) {
        this.imap.setFlags(seqno, flag, (err) => {
            if (!err) {
                this.adapter.log.info(seqno + ": setFlags success");
            } else {
                this.emit("error", err, this.client);
            }
        });
